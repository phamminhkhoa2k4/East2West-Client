  ...
  return (
    <>
      ...
      >
        ...
        {files.length > 0 && (
          <div
            className={`grid grid-cols-2 gap-10 mx-5 ${
              files.length > 2
                ? "scroll-transparent overflow-y-scroll h-[27.5rem]"
                : ""
            } `}
          >
            {files.map((file, index) => (
              <div key={index} className="relative">
                <Image
                  src={file.preview}
                  alt={file.name}
                  className="w-full h-59 object-cover object-center rounded-xl"
                  height={400}
                  width={400}
                />
                <button
                  onClick={(event) => removeFile(file.name, event)}
                  className="absolute top-3 right-3 bg-black text-white rounded-full p-2 text-xs"
                >
